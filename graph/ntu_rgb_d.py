...
class AdjMatrixGraph:
    def __init__(self, labeling_mode='spatial'):
        ...
        self.A_spatial = tools.get_spatial_graph(self.num_node[0], self.self_link[0], self.inward[0], self.outward[0])

        self.A_binary = [tools.get_adjacency_matrix(self.inward[i] + self.outward[i], self.num_node[i]) for i in range(len(self.num_node))]
        self.A_binary_with_I = [tools.get_adjacency_matrix(self.inward[i] + self.outward[i] + self.self_link[i], self.num_node[i])  for i in range(len(self.num_node))]
        self.A_norm = [tools.normalize_adjacency_matrix(self.A_binary_with_I[i])  for i in range(len(self.num_node))]
...
if __name__ == '__main__':
    graph = AdjMatrixGraph('spatial')
    A = graph.get_adjacency_matrix()
    P = graph.get_partion_metrix()

    import matplotlib.pyplot as plt
    A_norm, A_binary, A_binary_with_I = graph.A_norm, graph.A_binary, graph.A_binary_with_I
    f, ax = plt.subplots(1, 3)
    ax[0].imshow(A_binary_with_I[0], cmap='gray')
    ax[1].imshow(A_binary[0], cmap='gray')
    ax[2].imshow(A_norm[0], cmap='gray')
    plt.show()
